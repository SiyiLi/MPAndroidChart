...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleRadarDataSet<? extends Entry>>>
    ...
    private void prepareMatrixValuePx() {
        ...
        //Log.d("LOUIS", "mDeltaX " + mDeltaX);
        //Log.d("LOUIS", "scaleX " + scaleX);
        float scaleY = (float) ((getHeight() - mOffsetTop - mOffsetBottom) / mDeltaY);
        //Log.d("LOUIS", "mDeltaY " + mDeltaY);
        //Log.d("LOUIS", "scaleY " + scaleY);
    ...
    protected void calcModulus() {
        ...
        mMatrixTouch.getValues(values); // mMatrixTouch是什么，何时赋初值的
        ...
        //Log.d("LOUIS", "calcModulus: scaleX " + values[Matrix.MSCALE_X]);
        //Log.d("LOUIS", "calcModulus: mXAxisLabelModulus " + mXLabels.mXAxisLabelModulus);
    }
    ...
    private void prepareYLabels() {
        ...
        if (mContentRect.width() > 10 && !isFullyZoomedOutY()) {
            ...
            Log.d("LOUIS", "mYChartMin " + mYChartMin);
            Log.d("LOUIS", "mYChartMax " + mYChartMax);

            if (!mInvertYAxis) { // 对于宝贝树，if为true
                yMin = mYChartMin;
                yMax = mYChartMax;
            } else {
        ...
        Log.d("LOUIS", "mYLabelCount " + labelCount);
        Log.d("LOUIS", "rawInterval " + rawInterval);
        double interval = Utils.roundToNextSignificant(rawInterval);
        Log.d("LOUIS", "interval " + interval);
        double intervalMagnitude = Math.pow(10, (int) Math.log10(interval));
        Log.d("LOUIS", "intervalMagnitude " + intervalMagnitude);
        int intervalSigDigit = (int) (interval / intervalMagnitude);
        Log.d("LOUIS", "intervalSigDigit " + intervalSigDigit);
    ...
    @SuppressLint("NewApi") protected void drawXLabels(float yPos) {
        ...
        for (int i = 0; i < mCurrentData.getXValCount(); i += mXLabels.mXAxisLabelModulus) {
            ...
            if (position[0] >= mOffsetLeft && position[0] <= getWidth() - mOffsetRight) {
                ...
                // 再一次简单粗暴
                String timeStr = mCurrentData.getXVals().get(i);
    ...
    public Highlight getHighlightByTouchPoint(float x, float y) {
        ...
        if (mCurrentData != null && mCurrentData.getDataSetByIndex(0) != null)
            xIndex = mCurrentData.getDataSetByIndex(0).getNearestEntryForXIndex(xIndex).getXIndex();
        else
            return null;
